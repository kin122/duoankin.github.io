...
**Term** 代表文本中的一个单词。这是搜索单元，即搜索的最小元素。
**Token** 文档进行索引的最小元素。  
...
全文搜索：The full text queries enable you to search analyzed text fields such as the body of an email.  
精确搜索：term-level queries do not analyze search terms. Instead, term-level queries match the exact terms stored in a field.  

之间的比对：
1. 精确搜索只能查询一个field，而全文搜索可以多个字段同时搜索
2. term是将搜索内容直接作为一个最小单元与token进行比对；全文搜索则是进行语法和分词器解析后，将解析到的token进行与文档索引后得到的token进行比对。
3. term查询方式更加简单，虽然也有token要素的算分，但是要素较少。而全文搜索涉及的token要素较多，且与分词器关联较大。
4. 从lucene源码实现的角度来说，term查询的实现类是termQuery，全文搜索使用的是QueryParser类，而QueryParser最终默认使用TermQuery进行多个Term的OR关系查询。
