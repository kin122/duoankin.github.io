...
1. 字段查询：默认的查询方式是`status:active`这样的方式。可以在字段中添加操作符，比如`title:(quick OR brown)`;也可以对字段使用匹配，`book.\*:(quick OR brown)`,注意转义符的使用；对被查询的文档是使用英文双引号可以达到 match_phrase 的效果，`author:"John Smith"`; exists 方法的使用则是这样`_exists_:title`
